    ...
    */
    ...
    public function loadHtml($str, $encoding = null)
    {
        ...
        try {
            ...
            // Use Masterminds\HTML5 if available; otherwise fallback to DOMDocument
            if (class_exists(HTML5::class)) {
                $html5 = new HTML5(["encoding" => "UTF-8", "disable_html_ns" => true]);
                $dom = $html5->loadHTML($str);

                // Normalize document structure (Masterminds/html5-php#166)
                ...
                // Fallback parser: directly use DOMDocument
                $doc = new DOMDocument("1.0", "UTF-8");
                $doc->preserveWhiteSpace = true;
                $doc->loadHTML($str, LIBXML_NOWARNING | LIBXML_NOERROR);
                $this->loadDOM($doc, $quirksmode);
            }
        } finally {
